...
struct Density(MyMatrix, MyMatrix, MyMatrix, MyMatrix, MyMatrix, MyMatrix)
...
impl Fluid {
    ...
    pub fn new_density(&mut self) {
        ...
        for i in 0..9 {
            ...
            let dot_prod_square = &dot_prod.component_mul(&dot_prod);
            self.eq_density[i] = ((&self.total_density * self.cell_consts[i].prob).component_mul(&(&self.ones + 3.0 * &dot_prod + 
            4.5 * dot_prod_square + &pre_calculated)));
    ...
    pub fn stream(&mut self) {
        ...
        let mut norm_density_e = MyMatrix::zeros(ROWS, COLUMNS);
        norm_density_e.copy_from(&self.norm_density[1]);

        self.norm_density[1] = norm_density_e.remove_column(COLUMNS - 1).insert_column(0, 0.0); //e
        self.set_column(0, 1, &new_density);

        let mut norm_density_ne = MyMatrix::zeros(ROWS, COLUMNS);
        norm_density_ne.copy_from(&self.norm_density[5]);

        self.norm_density[5] = norm_density_ne.remove_row(0).remove_column(COLUMNS - 1).insert_row(ROWS - 1, 0.0) //ne 5
        .insert_column(0, 0.0);


        let mut norm_density_n = MyMatrix::zeros(ROWS, COLUMNS);
        norm_density_n.copy_from(&self.norm_density[2]);

        self.norm_density[2] = norm_density_n.remove_row(0).insert_row(ROWS - 1, 0.0); //n

        let mut norm_density_nw = MyMatrix::zeros(ROWS, COLUMNS);
        norm_density_nw.copy_from(&self.norm_density[6]);

        self.norm_density[6] = norm_density_nw.remove_row(0).remove_column(0).insert_row(ROWS - 1, 0.0)//nw 6
        .insert_column(COLUMNS - 1, 0.0);
        self.set_column(0, 6, &new_density);

        let mut norm_density_w = MyMatrix::zeros(ROWS, COLUMNS);
        norm_density_w.copy_from(&self.norm_density[3]);

        self.norm_density[3] = norm_density_w.remove_column(0).insert_column(COLUMNS - 1, 0.0); //w
        //self.set_column(0, 3, &new_density);

        let mut norm_density_sw = MyMatrix::zeros(ROWS, COLUMNS);
        norm_density_sw.copy_from(&self.norm_density[7]);

        self.norm_density[7] = norm_density_sw.remove_row(ROWS - 1).remove_column(0).insert_row(0, 0.0) //sw 7
        .insert_column(COLUMNS - 1, 0.0);
        self.set_column(0, 7, &new_density);

        let mut norm_density_s = MyMatrix::zeros(ROWS, COLUMNS);
        norm_density_s.copy_from(&self.norm_density[4]);

        self.norm_density[4] = norm_density_s.remove_row(ROWS - 1).insert_row(0, 0.0); //s

        let mut norm_density_se = MyMatrix::zeros(ROWS, COLUMNS);
        norm_density_se.copy_from(&self.norm_density[8]);

        self.norm_density[8] = norm_density_se.remove_row(ROWS - 1).remove_column(COLUMNS - 1).insert_row(0, 0.0) //se 8
        .insert_column(0, 0.0);
        self.set_column(0, 8, &new_density);
